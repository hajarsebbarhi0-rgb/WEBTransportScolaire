...
<div class="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50 text-[#346693] min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <h1 class="text-5xl font-extrabold text-center mb-12 uppercase tracking-tight font-serif text-[#346693]">
            ✏️ Modifier un Transport
        </h1>

        <div class="max-w-2xl mx-auto bg-white rounded-2xl p-8 shadow-xl border border-gray-200 border-t-4 border-t-[#346693]">
            
            <form action="{{ route('admin.transports.update', $transport->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="marque" class="block text-sm font-semibold mb-1 text-[#346693]">🚘 Marque</label>
                    <input type="text" id="marque" name="marque" value="{{ $transport->marque }}" 
                ...
                <div>
                    ...
                    <input type="text" id="modele" name="modele" value="{{ $transport->modele }}" 
                            class="w-full rounded-lg px-4 py-2 border border-gray-300 focus:ring-4 focus:ring-[#82D2F5]/50 transition duration-200 placeholder-gray-500" 
                            required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="plaque_immatriculation" name="plaque_immatriculation" value="{{ $transport->plaque_immatriculation }}" 
                            class="w-full rounded-lg px-4 py-2 border border-gray-300 focus:ring-4 focus:ring-[#82D2F5]/50 transition duration-200 placeholder-gray-500" 
                            required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="capacite_passagers" name="capacite_passagers" value="{{ $transport->capacite_passagers }}" 
                            class="w-full rounded-lg px-4 py-2 border border-gray-300 focus:ring-4 focus:ring-[#82D2F5]/50 transition duration-200 placeholder-gray-500" 
                            required>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" 
                            ...
                            required>
                        <option value="disponible" {{ $transport->status == 'disponible' ? 'selected' : '' }}>✅ Disponible</option>
                        <option value="en_service" {{ $transport->status == 'en_service' ? 'selected' : '' }}>🚍 En service</option>
                        <option value="en_maintenance" {{ $transport->status == 'en_maintenance' ? 'selected' : '' }}>🛠️ En maintenance</option>
                    </select>
                </div>
                ...
                <div class="flex justify-between pt-6">
                    ...
                    {{-- Mettre à jour : Fond Bleu Principal, hover en Cyan Secondaire --}}
                    <button type="submit" 
